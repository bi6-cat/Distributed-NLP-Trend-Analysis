...
# ============================================================================
# EXPORT HELPER — stg_keyword_freq schema
# ============================================================================

def export_keyword_freq_csv(
    cms: CountMinSketch,
    candidates: List[str],
    output_path: str,
    window_start: str,
    ...
    """
    Xuất kết quả CMS ra CSV theo schema `stg_keyword_freq` (ClickHouse).

    Schema output:
        keyword         String
        window_start    DateTime   (ISO 8601: "2026-04-29T00:00:00Z")
        window_end      DateTime
        estimated_count Int64
        source          String

    Args:
        cms: CountMinSketch đã được add() xong.
        candidates: Danh sách từ cần query (unique words đã thấy trong stream).
        output_path: Đường dẫn file CSV output.
        window_start: ISO string của đầu cửa sổ thời gian.
        window_end: ISO string của cuối cửa sổ thời gian.
        source: Nguồn dữ liệu ('voz','tinhte','all'...).
        top_k: Chỉ lưu top K từ có tần suất cao nhất.

    Example:
        >>> from algorithms.count_min_sketch import CountMinSketch, export_keyword_freq_csv
        >>> cms = CountMinSketch(d=5, w=2048)
        >>> for word in tokens: cms.add(word)
        >>> export_keyword_freq_csv(
        ...     cms, list(word_set),
        ...     output_path="output/stg_keyword_freq.csv",
        ...     window_start="2026-04-29T00:00:00Z",
        ...     window_end="2026-04-29T01:00:00Z",
        ...     source="voz",
        ... )
    """
    import csv
    ...
    # Query tất cả candidates và sort theo tần suất giảm dần
    rows = []
    for word in candidates:
        count = cms.query(word)
        if count > 0:
    ...
    with open(output_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(
            f,
            fieldnames=["keyword", "window_start", "window_end", "estimated_count", "source"],
        )
        writer.writeheader()
