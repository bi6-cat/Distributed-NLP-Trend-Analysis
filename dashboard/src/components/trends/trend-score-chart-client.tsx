...
export function TrendScoreChartClient({ data }: { data: TrendData[] }) {
  const safeData = data.length > 0 ? data : [
    { time: "00:00", score: 0 }
  ];
  
  const peak = Math.max(...safeData.map((d) => Number(d.score) || 0));
    ...
    <div className="rounded-xl bg-white border border-slate-200/80 shadow-[0_1px_2px_rgba(15,23,42,0.04)] flex flex-col h-[340px]">
      ...
      <div className="flex-1 px-3 pb-4">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={safeData} margin={{ top: 6, right: 12, left: -10, bottom: 0 }}>
            <defs>
